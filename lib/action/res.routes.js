...
function() {
    ...
    var rewriteRoutes = function(from, to) {
        w.parser.html.rewriteUrls(w.solution.resource, '.' + from + '/', '.' + to + '/');
        var routes = new io.File(w.solution.uri.combine('include.routes.js'));
        if (routes.exists()) {
        ...
        new io.File(w.solution.resource.uri).write(w.solution.resource.source);
    }
    
    var referenceResources = (function(){
        
        return function(){
            var globals = JSON.parse(new io.File(io.env.applicationDir.combine('globals.json')).read()),
            ...
            for(var key in routes){
                ...
                w.referenceHelper.create(new net.URI(w.solution.uri.toDir()), projectName, reference)
            }
    ...
    var importResources = (function() {
        ...
        return function() {
            processCopy(w.solution.resource);
            return 1;
        };
    })();
