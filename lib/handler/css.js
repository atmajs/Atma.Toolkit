    ...
    include.promise('handler').CssHandler = Class({
        Construct: function(solutionUri, outputDirectoryUri, resource){
            ...
            var copyImages = !urlhelper.isSubDir(solutionUri.toString(), resource.uri.toString()),
                ...
                path = outputDirectoryUri;
                
            io.settingsStyle.copyToOnlyImages = true;
            
            resource.source = new io.File(resource.uri).copyTo(path).source;
            return;
            ...
            for(var i = 0, x, length = images.length; x = images[i], i<length; i++){
                ...
                if (copyImages){
                    ...
                    console.log('copy image', x.uri.toLocalFile(), uri.toLocalFile(), resource, path.toLocalFile());
                    throw new Error();
