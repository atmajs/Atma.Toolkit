...
function() {
    ...
    window.io.fileFactory.registerHandler(/include\.loader\.js$/g, Class({
        Base: io.File,
        copyTo: function(uri){
            var source = this.read(),
                globals = JSON.parse(new io.File(io.env.applicationDir.combine('globals.json')).read()),
            ...
            for(var key in globals.defaultRoutes){
                ...
                var item = ruqq.arr.first(globals.projects, 'name','==', projectName);
                if (!(item && item.path)) {
            ...
            new io.File(uri).write(source.replace('%ROUTES%', JSON.stringify(globals.defaultRoutes)));
        }
    }));    
}();
