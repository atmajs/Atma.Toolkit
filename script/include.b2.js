	...
	var cfg = {},
		...
		helper = {			
			...
			each: function(arr, fn) {
				if (arr == null) return;
				if (arr instanceof Array) for (var i = 0; i < arr.length; i++) fn(arr[i], i);
				else fn(arr, 0);
			},
			/**
			 *	@arg x :
			...
			count: function(arr) {
				return arr instanceof Array? arr.length : (arr == null ? 0 : 1);
			},
		...
		resolve: function() {
			...
			for (var key in this.dfrCallbacks) {
				...
				if (typeof events[key] != 'function') {
					var r = callback.apply(this, [response]);
					if (r != null) response = response;
				} else {
